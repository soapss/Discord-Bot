...
public class GiveRole extends AdminCommand {
    ...
    public void exe(Guild guild, User user, TextChannel tc, Message msg) {
        ...
        if(!s.isPresent()) {
            ...
            //get any existing roles in the server with the role name above
        ...
        if(roles.isEmpty()){
            ...
            //add role to member
        guild.addRoleToMember(toGive,roles.get(0)).queue();
            //react to command
